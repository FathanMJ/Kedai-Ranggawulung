...
if (isset($_POST['make'])) {
    ...
    if (empty($_POST["order_code"]) || empty($_POST["customer_name"]) || empty($_GET['prod_price']) || empty($_POST['meja_id'])) {
        ...
        if ($postStmt) {
            ...
            updateProductStock($prod_id, $prod_qty);
...
function updateProductStock($prod_id, $prod_qty) {
    global $mysqli;
    $query = "UPDATE rpos_products SET prod_stock = prod_stock - ? WHERE prod_id = ?";
